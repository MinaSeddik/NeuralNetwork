...
public class BinaryCrossEntropyLoss extends LossFunction {

    private final static Logger logger = LoggerFactory.getLogger(BinaryCrossEntropyLoss.class);

    @Override
    public double[][] errorCost(double[][] labels, double[][] output) {
        double[][] costs = new double[labels.length][1];

        double yPrime, yPrimeComplement;
        for (int i = 0; i < labels.length; i++) {
            for (int j = 0; j < labels[0].length; j++) {
                yPrime = output[i][j] == 0d ? Double.MIN_VALUE : output[i][j];
                yPrimeComplement = ((1d - output[i][j]) == 0d) ? Double.MIN_VALUE : 1d - output[i][j];
                costs[i][0] += (labels[i][j] * Math.log(yPrime)) + ((1d - output[i][j]) * Math.log(yPrimeComplement));
            }
            costs[i][0] = -costs[i][0];
        }
    ...
    public double errorCost(Pair<Vector, Vector> pVector) {
        ...
        double sum = IntStream.range(0, y.length)
                .mapToDouble(i -> {
                    ...
                    double yPrimeComplementVal = ((1d - yPrime[i]) == 0d) ? Double.MIN_VALUE : (1d - yPrime[i]);
                    return y[i] * Math.log(yPrimeVal) + ((1d - yPrime[i]) * Math.log(yPrimeComplementVal));
                }).sum();
